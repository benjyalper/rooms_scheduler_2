...
try {
    ...
    // Simple test query
    const [results, fields] = await connection.execute('SELECT * FROM selected_dates_v2');
...
app.post('/submit', async (req, res) => {
    try {
        ...
        try {
            ...
            await connection.execute('INSERT INTO selected_dates_v2 (selected_date, names, color, startTime, endTime, roomNumber, recurringEvent) VALUES (?, ?, ?, ?, ?, ?, ?)', [selectedDate, names, selectedColor, startTime, endTime, roomNumber, recurringEvent]);
            ...
            if (recurringEvent) {
                ...
                for (let i = 1; i <= 4; i++) {
                    ...
                    await connection.execute('INSERT INTO selected_dates_v2 (selected_date, names, color, startTime, endTime, roomNumber, recurringEvent) VALUES (?, ?, ?, ?, ?, ?, ?)', [nextDate, names, selectedColor, startTime, endTime, roomNumber, recurringEvent]);
                }
            }
...
app.delete('/deleteEntry', async (req, res) => {
    ...
    try {
        ...
        await connection.execute('DELETE FROM selected_dates_v2 WHERE roomNumber = ? AND startTime = ?', [roomNumber, startTime]);
...
app.get('/room/:roomNumber', async (req, res) => {
    ...
    try {
        ...
        const connection = await pool.getConnection();
        const [roomRows] = await connection.execute('SELECT * FROM selected_dates_v2 WHERE roomNumber = ?', [roomNumber]);

        // Fetch data for today
        const nowMoment = moment().format('YYYY-MM-DD');
        const [dateRows] = await connection.execute('SELECT names, color, startTime, endTime, roomNumber FROM selected_dates_v2 WHERE selected_date = ?', [nowMoment]);
...
app.get('/fetchDataByDate', async (req, res) => {
    try {
        ...
        const connection = await pool.getConnection();
        const [rows] = await connection.execute('SELECT names, color, startTime, endTime, roomNumber FROM selected_dates_v2 WHERE selected_date = ?', [lookupDate]);
        connection.release();
...
app.get('/dateData', async (req, res) => {
    try {
        ...
        const connection = await pool.getConnection();
        const [rows] = await connection.execute('SELECT names, color, startTime, endTime, roomNumber FROM selected_dates_v2 WHERE selected_date = ?', [nowMoment]);
        connection.release();
    ...
    try {
        ...
        try {
            await connection.execute(
                'INSERT INTO selected_dates_v2 ( roomNumber, startTime, endTime) VALUES (?, ?, ?)',
...
app.post('/deleteRow', async (req, res) => {
    try {
        ...
        try {
            ...
            await connection.execute('DELETE FROM selected_dates_v2 WHERE roomNumber = ? AND startTime = ? AND endTime = ?', [roomNumber, startTime, endTime]);
